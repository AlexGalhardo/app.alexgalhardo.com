...
class Books {
    ...
    static async getRandom() {
        try {
            ...
            if (SESSION_USER) {
                database.books[random_book_index].userLoggedRecommend =
                    await Books.verifyIfLoggedUserRecommendThisBook(
                        SESSION_USER.id,
                        random_book_index + 1
                    );

                database.books[random_book_index].userLoggedNotRecommend =
                    await Books.verifyIfLoggedUserNotRecommendThisBook(
                        SESSION_USER.id,
                        random_book_index + 1
                    );
            } else {
        ...
        try {
            for (let i = 0; i < database.books_recommendations.length; i++) {
                if (
                    ...
                    database.books_recommendations[i].book_id ===
                    parseInt(book_id)
                ) {
        ...
        try {
            for (let i = 0; i < database.books.length; i++) {
                if (database.books[i].id === bookObject.id) {
                    if (bookObject.title)
                        database.books[i].title = bookObject.title;
                    if (bookObject.year_release)
                        database.books[i].year_release =
                            bookObject.year_release;
                    if (bookObject.image)
                        database.books[i].image = bookObject.image;
                    if (bookObject.amazon_link)
                        database.books[i].amazon_link = bookObject.amazon_link;
                    if (bookObject.resume)
                        database.books[i].resume = bookObject.resume;
                    if (bookObject.pages)
                        database.books[i].pages = bookObject.pages;
                    if (bookObject.genres)
                        database.books[i].genres = bookObject.genres;
                    if (bookObject.author)
                        database.books[i].author = bookObject.author;
    ...
    static searchTitle(book_title) {
        try {
            const searchedBooks = database.books.filter(
                (book) =>
                    book.title.toLowerCase().indexOf(book_title.toLowerCase()) >
                    -1
            );
        ...
        try {
            ...
            for (let i = 0; i < database.books_recommendations.length; i++) {
                if (
                    ...
                    if (database.books_recommendations[i].user_not_recommend) {
                        database.books_recommendations[i].user_not_recommend =
                            false;
                    ...
                    if (!database.books_recommendations[i].user_recommend) {
                        ...
                        database.books_recommendations[i].user_recommend =
                            false;
