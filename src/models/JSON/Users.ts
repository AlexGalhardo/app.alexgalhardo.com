...
class Users {
    ...
    static verifyIfEmailIsConfirmed(email) {
        try {
            for (let i = 0; i < database.users.length; i++) {
                if (
                    database.users[i].email == email &&
                    database.users[i].confirmed_email
                ) {
                    return true;
                }
            }
    ...
    static async verifyLogin(email, password) {
        try {
            for (let i = 0; i < database.users.length; i++) {
                if (database.users[i].email === email) {
                    const passwordValid = await Bcrypt.comparePassword(
                        password,
    ...
    static async create(userObject) {
        try {
            ...
            const passwordHash = await Bcrypt.cryptPassword(
                userObject.password
            );
        ...
        try {
            for (let i = 0; i < database.users.length; i++) {
                if (SESSION_USER) {
                    ...
                    userObject.id = await Users.getUserIDByEmail(
                        userObject.email
                    );
                }
                ...
                if (database.users[i].email === userObject.email) {
                    ...
                    if (userObject.name)
                        database.users[i].name = userObject.name;
                    if (userObject.new_email)
                        database.users[i].email = userObject.new_email;
                    if (userObject.document)
                        database.users[i].document = userObject.document;
                    if (userObject.phone)
                        database.users[i].phone = userObject.phone;
                    if (userObject.birth_date)
                        database.users[i].birth_date = userObject.birth_date;
                    if (userObject.zipcode)
                        database.users[i].address.zipcode = userObject.zipcode;
                    if (userObject.street)
                        database.users[i].address.street = userObject.street;
                    if (userObject.street_number)
                        database.users[i].address.street_number =
                            userObject.street_number;
                    if (userObject.neighborhood)
                        database.users[i].address.neighborhood =
                            userObject.neighborhood;
                    if (userObject.city)
        ...
        try {
            for (let i = 0; i < database.users.length; i++) {
                if (database.users[i].id === user_id) {
                    ...
                    database.users[i].stripe.card_exp_month =
                        cardObject.exp_month;
                    database.users[i].stripe.card_exp_year =
                        cardObject.exp_year;
                    Users.save(database);
        ...
        try {
            for (let i = 0; i < database.users.length; i++) {
                if (database.users[i].id === user_id) {
                    ...
                    database.users[i].stripe.currently_subscription_name =
                        plan_name;
                    database.users[i].stripe.subscription_start =
                        subscriptionObject.current_period_start;
                    database.users[i].stripe.subscription_end =
                        subscriptionObject.current_period_end;
                    database.users[i].stripe.cancel_at_period_end =
