...
export default class StripeModel {
	static async createShopTransaction (shopTransactionObject: inputShopTransactionObject) {
		...
		console.log('\n\n ENTROU => inputShopTransactionObject => ', shopTransactionObject)

		await prisma.stripeShopTransaction.create({
			data: {
				transaction_id: shopTransactionObject.transaction_id,
				total_amount: Math.floor(Number(shopTransactionObject.total_amount) * 100),
				...
				shipping_fee: Number(shopTransactionObject.shipping_fee) * 100,
			},
		});
	}
