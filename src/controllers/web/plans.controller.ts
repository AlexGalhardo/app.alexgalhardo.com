...
export default class PlansController {
	static async getViewPlans(req: Request, res: Response) {
		try {
			return res.setHeader("Content-Type", "text/html").end(
				await edge.render("pages/plans/plans", {
	...
	static async postSubscription(req: Request, res: Response, next: NextFunction) {
		try {
			...
			await UsersRepository.createStripeSubscription(global.SESSION_USER.id, subscriptionTransactionObject);
