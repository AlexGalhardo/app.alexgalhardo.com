...
export default class BlogController {
	...
	static async getSearchBlogTitle(req: Request, res: Response) {
		try {
			...
			const searchBlogTitle = req.query.blogTitle as string
			...
			if (!posts.length) {
				req.flash("warning", `No blog posts found from search: ${searchBlogTitle}!`);
				return res.redirect("/blog");
			}

			const totalBlogPostsFoundFromSearch = posts.length;
